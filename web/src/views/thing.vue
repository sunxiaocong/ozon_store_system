<template>
  <div>
    ...
    <div class="page-view">
      ...
      <!-- 显示总体积和总成本 -->
      <div class="summary-info" style="margin: 16px 0;">
        ...
        <div style="padding-right: 16px; max-height:480px; overflow-x: hidden;overflow-y: auto;">
          <a-form ref="myform" :label-col="{ style: { width: '80px' } }" :model="modal.form" :rules="modal.rules">
            <a-row :gutter="24">
              ...
              <a-col span="12">
                <a-form-item label="采购价格" name="price">
              ...
              <a-col span="12">
                <a-form-item label="进货数" name="purchase">
                  <a-input placeholder="请输入" v-model:value="modal.form.purchase"></a-input>
                </a-form-item>
              </a-col>
              <a-col span="12">
...
const columns = reactive([
  ...
  {
    title: '总成本明细',
    dataIndex: 'all_cost_detail',
    key: 'all_cost_detail',
    customRender: ({text}) => {
      // 使用 h 函数创建一个 div，并将换行符替换为 <br>
      return h('div', {
        innerHTML: text // 替换换行符
      });
    }
  },
  {
...
const totalVolume = computed(() => {
  ...
  data.dataList?.forEach(item => {
    sum += Number(item.volume) * Number(item.property);
  });
...
const totalCost = computed(() => {
  let sum = 0;
  data.dataList?.forEach(item => {
    sum += Number(item.all_cost) * Number(item.property);
  });
  return sum.toFixed(2);
});
...
const modal = reactive({
  ...
  rules: {
    ...
    purchase: [{required: true, message: '请输入', trigger: 'change'}],
...
const handleOk = () => {
  myform.value
      ...
      .then(() => {
        ...
        formData.append('purchase', modal.form.purchase || '')
