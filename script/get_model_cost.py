...
if __name__ == '__main__':
    ...
    cost_type = {
        "Трафареты": "0",  # 模版费用
        "Продвижение в поиске": "1",  # 搜索费用
        "Приобретение отзывов на платформе": "2",  # 计费评论
        "Подписка Premium": "3",  # 超级卖家
        "Услуга размещения товаров на складе": "4"  # 仓储安置费用
    }
    ...
    for store_name, ozon_auth in ozonIds.items():
        ...
        for info in res["result"]["operations"]:
            print(info["operation_type_name"], info["amount"])

            if info["operation_type_name"] == "Трафареты":  # 模版费用
                amount = info["amount"]
                all_amount += amount
            if info["operation_type_name"] == "Продвижение в поиске":  # 搜索费用
                amount = info["amount"]
                all_amount_1 += amount
            if info["operation_type_name"] == "Приобретение отзывов на платформе":  # 计费评论
                amount = info["amount"]
                all_amount_2 += amount
            if info["operation_type_name"] == "Подписка Premium":  # 超级卖家
                amount = info["amount"]
                all_amount_3 += amount
            if info["operation_type_name"] == "Услуга размещения товаров на складе":  # 仓储安置费用
                amount = info["amount"]
                all_amount_4 += amount
        cost_data_list = [
            (store_name, "0", date_str, all_amount),
            (store_name, "1", date_str, all_amount_1),
            (store_name, "2", date_str, all_amount_2),
            (store_name, "3", date_str, all_amount_3),
            (store_name, "4", date_str, all_amount_4)
        ]
        print(cost_data_list)
        insert_ozon_cost(cost_data_list)
